...
plant_group1(:,1)=plant_group1(:,1)+13;
...
% figure(1);
% c=tar_car(:,3);
% scatter3(tar_car(:,1),tar_car(:,2),tar_car(:,3),200,c,'.')
% axis([0 6 0 6 0 6])
% grid on
% h = colorbar;%右侧颜色栏
% set(get(h,'label'),'string','散射点高度');%给右侧颜色栏命名
% view(30,10)

figure(1);
a=car_group(:,4);
scatter3(car_group(:,1),car_group(:,2),car_group(:,3),100,a,'.')
hold on
b=plant_group(:,4);
scatter3(plant_group(:,1),plant_group(:,2),plant_group(:,3),100,b,'.')
hold on
c=light_group(:,4);
scatter3(light_group(:,1),light_group(:,2),light_group(:,3),100,c,'.')
hold on
d=people_group(:,4);
scatter3(people_group(:,1),people_group(:,2),people_group(:,3),100,d,'.')
axis([0 28 0 20 0 20])
grid on
h = colorbar;%右侧颜色栏
