...
Pfa = 1e-3; % 虚警概率，经典值
% 结果输出控制
threshold_adjust = 500000;
point_richness = 4;% 可以根据角度分辨率调整
% 创建平方律检测矩阵
detect_matrix_abs = abs(detect_matrix).^2 ;%.* abs(detect_matrix);% 平方律检测器
% 创建检测器
detector = phased.CFARDetector2D('GuardBandSize',guard_window_size,'TrainingBandSize',window_size,'ProbabilityFalseAlarm',Pfa,'ThresholdOutputPort',true);
% 寻找所有训练单元落在检测矩阵中的CUT坐标
Ngc = detector.GuardBandSize;
Ngr = detector.GuardBandSize;
Ntc = detector.TrainingBandSize;
Ntr = detector.TrainingBandSize;

cutidx = [];
colstart = Ntc + Ngc + 1;
colend = size(detect_matrix_abs, 2) - ( Ntc + Ngc);
rowstart = Ntr + Ngr + 1;
rowend = size(detect_matrix_abs, 1) - ( Ntr + Ngr);
...
% 检测
[dets,th] = detector(detect_matrix_abs,cutidx);
for i=1:size(dets, 1)
    threshold_matrix(cutidx(1, i), cutidx(2, i)) = th(i, 1);
    if dets(i, 1)>0
        target_index = [target_index; cutidx(1, i) cutidx(2, i) detect_matrix_abs(cutidx(1, i), cutidx(2, i))];
    end
end
